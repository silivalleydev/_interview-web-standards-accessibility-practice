...
import { InputLabel, MenuItem, Select, TablePagination, TextField } from '@material-ui/core'
import { downloadActivityLog, getActivityLog } from '../api/log'
...
const UserActivityLogPage: React.FC = () => {
  ...
  const handleGetActivityUser = async (newPage) => {
    ...
    if (result.status === 'OK') {
      ...
      setTotalPageCount(result.dataList.totalDataCount)
  ...
  return (
    <>
      ...
      <Paper
        ...
        style={{ backgroundColor: '#F7F8FA', marginBottom: 40 }}
      >
        <TableContainer>
          {
            isLoading
            ...
            <TableBody>
              {rows.map((row, idx) => (
                ...
                >
                  ...
                  <TableCell align="center">{row.uriMenu}</TableCell>
                  <TableCell align="center">{row.userUid}</TableCell>
                  <TableCell align="center">{row.classification}</TableCell>
                  <TableCell align="center">{row.activityResult?.replace('null', '') || ''}</TableCell>
                  <TableCell align="center">{row.downloadReason?.replace('null', '') || ''}</TableCell>
